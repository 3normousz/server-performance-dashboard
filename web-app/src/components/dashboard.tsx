...
import { fetchMetrics } from "@/app/utils/fetchMetrics";

interface DashboardProps {
  serverAddress: string;
}

export function Dashboard({ serverAddress }: DashboardProps) {
  ...
  const fetchAll = async () => {
    const cpuUsage = await fetchMetrics(`sum by (mode)(irate(node_cpu_seconds_total{instance="${serverAddress}", job="linux"}[15s]))
      / scalar(count(count(node_cpu_seconds_total{instance="${serverAddress}", job="linux"}) by (cpu)))* 100`);
    const cpuIdlePercentage = await fetchMetrics(`100 * (1 - avg(rate(node_cpu_seconds_total{mode="idle", instance="${serverAddress}"}[15s])))`);
    const mem = await fetchMetrics(`(1 - (node_memory_MemAvailable_bytes{instance="${serverAddress}", job="linux"} / node_memory_MemTotal_bytes{instance="${serverAddress}", job="linux"})) * 100`);
    const totalMem = await fetchMetrics(`node_memory_MemTotal_bytes{instance="${serverAddress}",job="linux"}`);
    const diskUsage = await fetchMetrics(`100 - ((node_filesystem_avail_bytes{instance="${serverAddress}",job="linux",device!~'rootfs'} * 100) / node_filesystem_size_bytes{instance="${serverAddress}",job="linux",device!~'rootfs'})`);
    const diskUsedPercentage = await fetchMetrics(`100 - ((node_filesystem_avail_bytes{instance="${serverAddress}",job="linux",mountpoint="/",fstype!="rootfs"} * 100) / node_filesystem_size_bytes{instance="${serverAddress}",job="linux",mountpoint="/",fstype!="rootfs"})`);
